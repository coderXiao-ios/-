...
+ (BOOL)findSubView:(Class)cls view:(UIView*)v allSameType:(BOOL)same container:(NSMutableArray *)container{
    ...
    if (!found) {
        [v.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            BOOL tmp = [self findSubView:cls view:obj allSameType:same container:container];
            found = found?YES:tmp;
            *stop = found && !same;
...
- (BOOL)findSubView:(Class)cls allSameType:(BOOL)same container:(NSMutableArray*)container{
    return [[self class] findSubView:cls view:self allSameType:same container:container];
}
